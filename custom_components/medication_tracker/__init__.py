...
def _register_services(hass: HomeAssistant, entry: MedicationTrackerConfigEntry) -> None:
    ...
    async def handle_mark_taken(call: ServiceCall) -> None:
        ...
        success = await coordinator.async_mark_taken(med_id, taken_at=taken_at, scheduled_time=scheduled_time)
        if not success:
    ...
    async def handle_reset_today(call: ServiceCall) -> None:
        ...
        if not success:
            raise ServiceValidationError(f"Could not reset today's log for medication '{med_id}'.")

    hass.services.async_register(DOMAIN, SERVICE_MARK_TAKEN, handle_mark_taken, schema=_MARK_TAKEN_SCHEMA)
    hass.services.async_register(DOMAIN, SERVICE_MARK_SKIPPED, handle_mark_skipped, schema=_MARK_SKIPPED_SCHEMA)
    hass.services.async_register(DOMAIN, SERVICE_RESET_TODAY, handle_reset_today, schema=_SERVICE_BASE_SCHEMA)
